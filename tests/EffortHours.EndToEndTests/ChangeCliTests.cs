...
public sealed partial class ChangeCliTests
{
    ...
    [Fact]
    public async Task GeneratedCustomCodeIsRepresentedWithoutEmittingItsSource()
    {
        using GitFixture repository = await GitFixture.CreateAsync();
        repository.WriteText("Demo.csproj", ProjectFile);
        repository.WriteText(
            "Generated.g.cs",
            GeneratedClient("v1", "customer-custom-secret-1"));
        _ = await repository.CommitAsync("base");
        repository.WriteText(
            "Generated.g.cs",
            GeneratedClient("v2", "customer-custom-secret-2"));
        string headObjectId = await repository.CommitAsync("customize generated client");
        ...
        JsonElement path = Assert.Single(report.RootElement
            ...
            .GetProperty("paths")
            .EnumerateArray());
        Assert.Equal("represented", path.GetProperty("classification").GetString());
        Assert.True(path.GetProperty("represented").GetBoolean());
        Assert.Contains(
            path.GetProperty("tags").EnumerateArray(),
            tag => tag.GetString() == "normalization:generated-customization-represented");
        Assert.Contains(
            report.RootElement.GetProperty("diagnostics").EnumerateArray(),
            diagnostic => diagnostic.GetProperty("code").GetString() == "FB5203");
    }
    ...
    private static string GeneratedClient(string version, string value) =>
        "// <auto-generated />\n" +
        $"// generated body {version}\n" +
        "namespace Demo;\n" +
        "// <custom-code>\n" +
        $"internal partial class Client {{ public string Value => \"{value}\"; }}\n" +
        "// </custom-code>\n";
