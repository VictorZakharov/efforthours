...
public sealed class PhpCliTests
{
    [Fact]
    public async Task ScanAndEstimateProduceStaticComposerPhpEvidenceWithoutSourceDisclosure()
    {
        using PhpRepository repository = new();
        repository.WriteText(
            "composer.json",
            "{\"name\":\"acme/app\",\"require\":{\"laravel/framework\":\"^12.0\"},\"autoload\":{\"psr-4\":{\"App\\\\\":\"app/\"}}}");
        repository.WriteText(
            "routes/api.php",
            "<?php\nuse Illuminate\\Support\\Facades\\Route;\nRoute::get('/synthetic-private-route-marker', fn () => true);\n");
        ...
        Assert.DoesNotContain("synthetic-private-route-marker", scan.StandardOutput, StringComparison.Ordinal);
        ...
        Assert.Contains(
            ...
            ecosystem => ecosystem.GetString() == "php");
        Assert.Contains(
    ...
    [Fact]
    public async Task ChangeUsesPhpNormalizationAndSemanticRoutingThroughTheCli()
    {
        using PhpRepository before = new();
        using PhpRepository after = new();
        before.WriteText("src/Service.php", "<?php\nfinal class Service { public function value(): string { return 'before'; } }\n");
        after.WriteText("src/Service.php", "<?php\nfinal class Service { public function value(): string { return 'after'; } }\n");
        ...
        Assert.Equal(
            "change-seed/0.16.0+seed-rules/0.4.0",
    ...
    private sealed class PhpRepository : IDisposable
    {
        public PhpRepository()
        {
            RootPath = Path.Combine(Path.GetTempPath(), "efforthours-php-e2e", Guid.NewGuid().ToString("N"));
        ...
        public void Dispose()
        {
            ...
            string expectedParent = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "efforthours-php-e2e"));
