...
{
    [Fact]
    public async Task ScanAndEstimateProduceStaticCargoRustEvidenceWithoutSourceDisclosure()
    {
        using RustRepository repository = new();
        ...
        repository.WriteText(
            ...
            "fn main() { let _ = Router::new().route(\"/synthetic-private-route-marker\", get(main)); }\n");
        ...
        Assert.Contains(
            ...
            ecosystem => ecosystem.GetString() == "rust");
        Assert.Contains(
    ...
    [Fact]
    public async Task ChangeUsesRustNormalizationAndSemanticRoutingThroughTheCli()
    {
        using RustRepository before = new();
        using RustRepository after = new();
        before.WriteText("src/service.rs", "pub fn value() -> &'static str { r#\"before\"# }\n");
        after.WriteText("src/service.rs", "pub fn value() -> &'static str { r#\"after\"# }\n");
    ...
    private sealed class RustRepository : IDisposable
    {
        public RustRepository()
        {
            RootPath = Path.Combine(Path.GetTempPath(), "efforthours-rust-e2e", Guid.NewGuid().ToString("N"));
        ...
        public void Dispose()
        {
            ...
            string expectedParent = Path.GetFullPath(Path.Combine(Path.GetTempPath(), "efforthours-rust-e2e"));
