...
public sealed class GoCliTests
{
    [Fact]
    public async Task ScanAndEstimateProduceStaticGoEvidenceWithoutSourceDisclosure()
    {
        using GoRepository repository = new();
        ...
        Assert.DoesNotContain("privateGoMarker", scan.StandardOutput, StringComparison.Ordinal);
        ...
        Assert.Contains(
            ...
            ecosystem => ecosystem.GetString() == "go");
    ...
    [Fact]
    public async Task ChangeUsesGoNormalizationAndSemanticRoutingThroughTheCli()
    {
        using GoRepository before = new();
        using GoRepository after = new();
        before.WriteText("go.mod", "module example.com/change\n");
        after.WriteText("go.mod", "module example.com/change\n");
        before.WriteText(
            "server.go",
            "package server\nimport \"net/http\"\nfunc register() {}\n");
        after.WriteText(
            "server.go",
            "package server\nimport \"net/http\"\nfunc register() { http.HandleFunc(\"/ready\", ready) }\nfunc ready(http.ResponseWriter, *http.Request) {}\n");
        ...
        Assert.Equal(
            "change-seed/0.10.0+seed-rules/0.4.0",
    ...
    private sealed class GoRepository : IDisposable
    {
        public GoRepository()
        {
