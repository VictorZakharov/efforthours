...
public sealed class ChangeCalibrationCliTests
{
            ...
            [
                new CalibrationReviewPlanRecord
                {
                    ...
                    Source = new CalibrationSourceProvenance
                    {
                        ...
                        SourceReference = "fairbill://e2e/change-calibration",
    ...
    private static async Task<ProcessResult> RunCliAsync(params string[] arguments)
    {
        ...
        string cli = Path.Combine(root, "src", "Fairbill.Cli", "bin", configuration, "net10.0", "fairbill.dll");
        ...
        using Process process = Process.Start(startInfo)
            ?? throw new InvalidOperationException("Could not start Fairbill CLI.");
    ...
    private static string FindRepositoryRoot()
    {
        ...
        while (current is not null && !File.Exists(Path.Combine(current.FullName, "Fairbill.slnx")))
        {
    ...
    private sealed class TemporaryDirectory : IDisposable
    {
        ...
        {
            RootPath = Path.Combine(Path.GetTempPath(), $"fairbill-change-calibration-{Guid.NewGuid():N}");
