...
internal sealed class PythonTextReader(
    ...
    string rootPath)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Failure(path, $"Python input '{path}' has no common-scanner content digest and was skipped.");
        }
        ...
        {
            return Failure(path, $"Python input '{path}' exceeds the eight-megabyte analysis limit and was skipped.");
        }
        ...
        {
            return Failure(path, $"Python input '{path}' resolves outside repository scope and was skipped.");
        }
        ...
        {
            return Failure(path, $"Python input '{path}' could not be read and was skipped.");
        }
        ...
        {
            return Failure(path, $"Python input '{path}' changed after common scanning; semantic evidence was skipped.");
        }
        ...
        try
        {
            ...
            return text.AsSpan().Contains('\0')
                ? Failure(path, $"Python input '{path}' contains binary null data and was skipped.")
                : new PythonTextReadResult(text, null);
        }
        catch (DecoderFallbackException)
        {
            return Failure(path, $"Python input '{path}' is not valid supported text and was skipped.");
        }
    }
    ...
    private static PythonTextReadResult Failure(string path, string message) => new(
        null,
        PythonEvidence.Diagnostic("FB7001", DiagnosticSeverity.Warning, message, path));
}

internal sealed record PythonTextReadResult(string? Text, Diagnostic? Diagnostic);
