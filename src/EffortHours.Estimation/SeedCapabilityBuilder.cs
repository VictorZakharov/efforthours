    ...
    SeedWorkItemFactory workItemFactory)
{
    ...
    private void AddBuildTooling(List<CapabilityUnit> capabilities)
    {
        SeedFileEvidence[] files = [.. _index.Files.Where(file =>
            ...
            !file.Ecosystems.Contains("terraform", StringComparer.Ordinal))];
        ...
        EvidenceFact[] ecosystemConfigurations = [.. _index.FactsOfKind(EvidenceKinds.BuildConfiguration)
            .Where(fact => fact.Provenance.Analyzer is
                ...
                "efforthours.rust-analyzer" or "efforthours.cpp-analyzer")];
        ...
        decimal configFiles = canonical.Count(file => file.Role == "build-configuration") +
            configurations.Length + ecosystemConfigurations.Length;
