...
internal static class BenchmarkFixtureGenerator
{
    ...
    public static void Generate(string rootPath, BenchmarkOptions options)
    {
        ...
        if (options.Shape is BenchmarkShape.JavaScript or BenchmarkShape.Frontend or BenchmarkShape.Mixed)
        {
            File.WriteAllText(
        ...
        Parallel.For(0, options.Files, index =>
        {
            ...
            string directory = Path.Combine(
                ...
                kind switch
                {
                    ...
                    BenchmarkSourceKind.Html or BenchmarkSourceKind.Css or BenchmarkSourceKind.Scss => "frontend",
                    BenchmarkSourceKind.Sql => "database",
                    _ => "web",
                },
                $"group-{index / 100:D5}");
            ...
            (string fileName, string content) = kind switch
            {
                ...
                BenchmarkSourceKind.Html => SupplementalBenchmarkFixtures.Html(index, options.LinesPerFile),
                BenchmarkSourceKind.Css => SupplementalBenchmarkFixtures.Css(index, options.LinesPerFile),
                BenchmarkSourceKind.Scss => SupplementalBenchmarkFixtures.Scss(index, options.LinesPerFile),
                BenchmarkSourceKind.Sql => SupplementalBenchmarkFixtures.Sql(index, options.LinesPerFile),
    ...
    private static BenchmarkSourceKind SourceKind(BenchmarkShape shape, int index) => shape switch
    {
        ...
        BenchmarkShape.JavaScript => index % 2 == 0
            ? BenchmarkSourceKind.JavaScript
            : BenchmarkSourceKind.TypeScript,
        BenchmarkShape.Frontend => (index % 3) switch
        {
            0 => BenchmarkSourceKind.Html,
            1 => BenchmarkSourceKind.Css,
            _ => BenchmarkSourceKind.Scss,
        },
        BenchmarkShape.Sql => BenchmarkSourceKind.Sql,
