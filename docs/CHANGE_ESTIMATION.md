...
`change-seed/0.9.0` rules remain experimental and are not empirically calibrated,
production-ready, or separately model-admitted. They preserve the valuation
behavior of the Stage A logically admitted 0.6.0 baseline; the SQL, Python, and Go
paths were not present in that gate.
Version 0.9.0 retains the 0.3.0 logical-marginality correction and the 0.4.0
...
implicit-semicolon/category routing without changing any existing Change EHE prior
...
`change-portfolio/0.1.0+change-seed/0.9.0+seed-rules/0.4.0` reconciler changes no
...
`change-seed/0.9.0+seed-rules/0.4.0`; the portfolio reconciler identity is
`change-portfolio/0.1.0+change-seed/0.9.0+seed-rules/0.4.0`. The earlier 0.6.0
Change identity alone passed the experimental Stage A logical gate, and that
record contains no SQL, Python, or Go. Portfolio aggregation does not broaden that
admission. Neither 0.9.0 nor portfolio 0.1.0 may be described as empirically calibrated,
