...
The current `change-seed/0.4.0` rules retain the 0.3.0 correction that keeps
...
a calibration result. The current composite source identity is
`change-seed/0.4.0+seed-rules/0.3.0` because the repository frontend model
advanced independently; Change rules and every frozen Change report remain
unchanged.
