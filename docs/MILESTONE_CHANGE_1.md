...
`change-seed/0.9.0+seed-rules/0.4.0`. Version 0.6.0 alone is admitted as the
experimental Stage A logical baseline for 4-to-32-hour changes; later revisions
preserve its admitted rules but add unadmitted SQL, Python, and Go paths. Its ranges are not formal
